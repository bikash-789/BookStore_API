...
def isAdmin():
    try:
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT registered_type FROM users WHERE email = %s', (current_user,))
    ...
    if request.is_json:
        ...
        if 'title' in data and 'author' in data and 'ISBN' in data and 'price' in data and 'quantity' in data:
            ...
            try:
                ...
                cursor.execute('SELECT * FROM books WHERE ISBN = %s', (book_ISBN,))
                book = cursor.fetchone()
                if book:
                    msg = 'Book already exists in the database!'
                else:
                    # Store the book details in the database
                    cursor.execute('INSERT INTO books (title, author, ISBN, price, quantity) VALUES (%s, %s, %s, %s, %s)', (book_title, book_author, book_ISBN, book_price, book_quantity))
                ...
                for book_data in data:
                    if 'title' in book_data and 'author' in book_data and 'ISBN' in book_data and 'price' in book_data and 'quantity' in book_data:
                        ...
                        if not book:
                            ...
                            cursor.execute('INSERT INTO books (title, author, ISBN, price, quantity) VALUES (%s, %s, %s, %s, %s)', (book_title, book_author, book_ISBN, book_price, book_quantity))
                            mysql.connection.commit()
                            success_messages.append(f'Book with ISBN - {book_ISBN} has been added to the database!')
                        else:
                            error_messages.append(f'Book with ISBN - {book_ISBN} already exists in the database!')

                    else:
                        error_messages.append('Please provide all required fields for each book in JSON format! (title, author, ISBN, price, quantity)')
