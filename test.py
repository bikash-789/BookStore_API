...
def get_auth_token():
    ...
    login_response = requests.post('http://localhost:3000/api/login', json=login_data)
    ...
    if login_response.status_code == 200:
        ...
        raise Exception(f'Authentication failed. Status code: {login_response.status_code}')
...
class BookStoreAPITests(unittest.TestCase):
    ...
    def test_04_delete_book_by_isbn(self):
        ...
        response = self.app.delete(f'http://localhost:3000/api/book/{isbn}', headers={'Authorization': f'Bearer {self.auth_token}'})
